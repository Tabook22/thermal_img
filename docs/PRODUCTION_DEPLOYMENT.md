...
Use `docker-compose.prod.yml`; `docker-compose.yml` remains a local development example. Production uses SQLite at `/data/thermal.db` inside the persistent `thermal_inspector_thermal_data` volume. Run `python3 deploy/create-production-env.py` once on the VPS to create a mode 600 `.env` with `DATABASE_URL=sqlite:////data/thermal.db` and `CORS_ORIGINS=https://skygreenline-lab.io`. The command refuses to overwrite an existing `.env`. Never commit `.env`. All published Compose ports bind to loopback. Install `deploy/nginx-thermal.conf` as a snippet in the existing HTTPS server block after backing up that server file. The snippet protects both thermal routes with Basic Authentication; `/api/` and `location /` stay with the existing site.
