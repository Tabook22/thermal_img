...
export function EnhancementLegend({control:c}:{control:EnhancementController}){
 ...
 const fullLow=minimum??p?.low,fullHigh=maximum??p?.high;
 const rawLow=c.settings.low??fullLow,rawHigh=c.settings.high??fullHigh;
 ...
 return createPortal(<div ref={legendRef} role="group" aria-label="Draggable thermal color legend with adjustable upper and lower display-temperature nodes" className={`colorbar enhancementLegend ${calibrated?'calibrated':''}`} style={position?{left:position.x,top:position.y}:undefined} title={calibrated?'Drag white space to move the box. Drag the red or blue arrow node vertically to adjust the display range.':'Drag the approximate color guide anywhere in the app window. Colors are not a measured temperature scale.'} onPointerDown={down} onPointerMove={move} onPointerUp={end} onPointerCancel={end}>{calibrated?<><div className="legendLimit legendHigh">{fullHigh!.toFixed(1)} <Info/></div><div className="legendRail"><div className="legendGradient" style={{top:`${highPosition}%`,bottom:`${100-lowPosition}%`,background:`linear-gradient(to top, ${colors.join(',')})`}}/><button type="button" className="legendMarker maximum" style={{top:`${highPosition}%`}} aria-label="Upper display-temperature node" title="Drag vertically to adjust the upper display temperature" onPointerDown={e=>startScale('high',e)} onPointerMove={moveScale} onPointerUp={endScale} onPointerCancel={endScale}><span>{displayHigh!.toFixed(1)}°C</span><i><b/><b/><b/></i></button><button type="button" className="legendMarker minimum" style={{top:`${lowPosition}%`}} aria-label="Lower display-temperature node" title="Drag vertically to adjust the lower display temperature" onPointerDown={e=>startScale('low',e)} onPointerMove={moveScale} onPointerUp={endScale} onPointerCancel={endScale}><i><b/><b/><b/></i><span>{displayLow!.toFixed(1)}°C</span></button></div><div className="legendLimit legendLow">{fullLow!.toFixed(1)}</div><small>Move box · drag nodes ↕</small></>:<><span>Warm</span><div className="legendSimple" style={{background:`linear-gradient(to top, ${colors.join(',')})`}}/><span>Cool</span><small>Color guide · drag</small></>}</div>,document.body);
}
